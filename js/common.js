...
(function (common) {
  ...
  */
  ...
  function loaderFadeout() {
    ...
    var startPoint = 0;
    var lastPath = 0;
    var loadingSvg = Snap().attr({ viewBox: [0, 0, canvasW, canvasH] });
    ...
    var circle = loadingSvg.circle(canvasW / 2, 3.5, 3).attr({ id: "circlePath", fill: "#222" });

    var points;
    var loopBase = loadingSvg.g().attr({ id: 'loopBase' });
    ...
    var drawingPath = loadingSvg.path().attr({ id: "drawingPath", path: subPath, fill: "none", stroke: "#222", strokeWidth: 2 });
  ...
  */
  ...
  function setEvents() {
    ...
    $('.header_menu, #header .closeBtn').on('click', function () {
      if ($('body').is('.navOpen')) {
        ...
        TweenLite.to($headerNav, 0.3, { top: 0, alpha: 0, ease: Power2.easeOut, onComplete: function onComplete() {
            $headerNav.css({ display: 'none' });
            $('.wrapper').css({ position: '', top: '', height: '' });
            $('html,body').scrollTop(backSc);
          } });
      } else {
        $('body').addClass('navOpen');
        TweenLite.to($headerNav, 0.3, { display: 'block', top: 50, alpha: 1, ease: Power2.easeOut, onComplete: function onComplete() {
            $('.wrapper').css({ position: 'fixed', top: -ride.scrollTop });
    ...
    mc.on('panstart panmove panup pandown panend', function (ev) {
      switch (ev.type) {
        ...
        case 'panend':
          ...
          TweenLite.to($projectBox, 0.3, { top: movePos, ease: Power2.easeOut, onComplete: function onComplete() {
              panPos = movePos;
            } });
          break;
      }
  ...
  function langChange() {
    ...
    var s = Snap().attr({ viewBox: [0, 0, canvasX, canvasY], id: "langChangeObj" });
    ...
    var knobGroup = s.g();
    var knob = knobGroup.rect(0, 0, canvasX / 2, canvasY).attr({ fill: "#999" });
    var knobText = knobGroup.rect(0, 0, canvasX / 2, canvasY).attr({ fill: "#fff", clipPath: langText });
